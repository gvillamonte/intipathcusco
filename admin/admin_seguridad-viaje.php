...
requierePermiso('seguridad_viaje');
...
if (isset($_POST['update_page'])) {
    ...
    if (isset($_FILES['aside_foto']) && $_FILES['aside_foto']['error'] == 0) {
        $aside_img = "seguridad_spec_" . time() . ".jpg";
        move_uploaded_file($_FILES['aside_foto']['tmp_name'], "../assets/img/seguridad-viaje/" . $aside_img);
    }
    $intro_img = $_POST['current_intro_img'];
    if (isset($_FILES['intro_imagen']) && $_FILES['intro_imagen']['error'] == 0) {
        $intro_img = "seguridad_banner_" . time() . ".jpg";
        move_uploaded_file($_FILES['intro_imagen']['tmp_name'], "../assets/img/seguridad-viaje/" . $intro_img);
    }
    $cta_img = $_POST['current_cta_img'];
    if (isset($_FILES['cta_foto']) && $_FILES['cta_foto']['error'] == 0) {
        $cta_img = "seguridad_cta_bg_" . time() . ".jpg";
        move_uploaded_file($_FILES['cta_foto']['tmp_name'], "../assets/img/seguridad-viaje/" . $cta_img);
    }
    $stmt = $db->prepare("UPDATE seguridad_viaje SET banner_titulo = ?, banner_titulo_en = ?, banner_subtitulo = ?, banner_subtitulo_en = ?, intro_texto = ?, intro_texto_en = ?, intro_imagen = ?, aside_texto = ?, aside_texto_en = ?, aside_btn = ?, aside_btn_en = ?, aside_imagen = ?, cta_titulo = ?, cta_titulo_en = ?, cta_texto = ?, cta_texto_en = ?, cta_btn = ?, cta_btn_en = ?, cta_imagen = ?,
    ...
    header("Location: admin_seguridad-viaje.php?res=updated");
    exit;
}
$config = $db->query("SELECT * FROM seguridad_viaje WHERE id = 1")->fetch(PDO::FETCH_ASSOC);
?>
<!DOCTYPE html>
<html lang="es">
<head>
    <meta charset="UTF-8"><title>Admin Seguridad Viaje | IntiPath</title>
...
<body>
    <div class="admin-wrapper"><?php include '../includes/sidebar.php'; ?>
        <main class="main-content" style="padding:30px;">
            <h1 class="admin-title-inti"><i class="fas fa-shield-alt"></i> Configuración - Seguridad en el Viaje</h1>
            ...
            <div class="current-img-preview">
                ...
                <img src="../assets/img/seguridad-viaje/<?= $config["intro_imagen"] ?>" style="max-width:200px;max-height:150px;border-radius:8px;border:2px solid #ddd;">
        ...
        <?php endif; ?><input type="hidden" name="current_intro_img" value="<?= $config['intro_imagen'] ?? '' ?>"></div>
            ...
            <div class="current-img-preview">
                <p style="margin:10px 0 5px;font-weight:600;color:#15305D;">📷 Imagen Actual:</p>
                <img src="../assets/img/seguridad-viaje/<?= $config["aside_imagen"] ?>" style="max-width:200px;max-height:150px;border-radius:8px;border:2px solid #ddd;">
        ...
        <?php endif; ?><input type="hidden" name="current_aside_img" value="<?= $config['aside_imagen'] ?? '' ?>"></div>
            ...
            <div class="current-img-preview">
                <p style="margin:10px 0 5px;font-weight:600;color:#15305D;">📷 Imagen Actual:</p>
                <img src="../assets/img/seguridad-viaje/<?= $config["cta_imagen"] ?>" style="max-width:200px;max-height:150px;border-radius:8px;border:2px solid #ddd;">
